...
`lib/design/untitled-theme.css`, `scripts/gen-tokens.mjs` and 1,353 lines of
token tests are **gone**. In their place:

```
styles/theme.css     tokens only, not one selector
styles/globals.css   imports, plugins, variants, base — ceiling 200, now 198
styles/app.css       what Tailwind cannot express — still empty
app/styles/poke-holo.css   kept: it is a feature, not a layer (ADR-0061)
```

Bart's instruction decided the method: *"Kapot is zichtbaar en oplosbaar.
Half-gemigreerd is onzichtbaar en sleept."* So it went out in one commit
(`chore: remove all CSS`), not incrementally. **ADR-0093** is the record.

What changed structurally, and it is more than a tidy-up:

- **No colour exists twice.** Untitled UI ships `@theme` plus `.dark-mode`;
  those are folded into one `light-dark()` per value. 324 doubled colours → 0,
  and the two-selector dark-mode bridge is gone entirely. 205 kB → 179 kB.
- **The generator runs the other way.** `theme.css` is the source and
  `scripts/extract-theme-values.mjs` resolves values *out* of it into
  `lib/design/theme-values.generated.ts`, for the four consumers that cannot
  read a stylesheet (manifest, `viewport.themeColor`, both OG images). `npm run
  tokens` writes it; `npm run check` fails on a hand edit.
- Card Orb's label colours are aliases of Untitled UI's text tokens now, so body
  text is `#171717` not `#111111`. Higher contrast.
- `tailwindcss-animate` and `tailwindcss-react-aria-components` are installed.
- `poke-holo.css` is in `@layer components`. It was unlayered, which meant every
  rule in it beat every utility in the app.

### The part worth reading before you touch a token

**ADR-0094.** Everything passed — Prettier, typecheck, 452 tests, lint, build,
and a 491-token comparison across four theme states in a real browser. Then the
app was loaded and the buttons were square.

Two failures, both silent:

1. ADR-0088's `shape-round`/`shape-rectangle` were `@utility` blocks the old
   generator emitted, so they died with it, along with six token families.
   **41 call sites referenced them.** A class that matches no rule is not an
   error in CSS, TypeScript or ESLint.
2. With the tokens restored, the button was *still* square, while
   `--radius-control` computed to 999px correctly. It had been put in `:root`
   instead of `@theme`, and **only a token inside `@theme` becomes a utility**.
   The variable resolved; `rounded-control` was never a class.

**Nothing in this repository's checks can catch either.** What catches them is
opening the app. If you change a token, render a screen.

### Still open

- **The measured contrast arguments are gone** with `lib/design/tokens.ts`.
  They are at `git show 22ca2cb~1:lib/design/tokens.ts` and nothing replaces
