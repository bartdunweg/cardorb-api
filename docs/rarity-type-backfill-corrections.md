# Rarity/type backfill: rows with no confident TCGdex match

A worklist, not a decision — the reasoning is in
`docs/decisions/0030-tcgdex-source-of-truth-for-rarity-and-type.md`. These rows kept
their existing rarity/type untouched, on the same principle ADR-0022 applied to
artwork: a wrong fact is worse than a missing one. Most of these are the same rows
listed in `trainer-gallery-row-corrections.md`, or promos TCGdex has never indexed.
Fix the underlying number/name mismatch (or accept there is nothing to match), then
re-run this script. Delete this file once it is empty.

## Battle Styles — 1 row

| number | name | current rarity | current types |
| --- | --- | --- | --- |
| 154 | Single Strike Urshifu | Rare Holo | V |

## Black Bolt — 2 rows

| number | name | current rarity | current types |
| --- | --- | --- | --- |
| 085 | Professor’s Research (Juniper) | Reversed Holo | — |
| 085 | Professor’s Research (Juniper) | Non-holo | — |

## Chilling Reign — 1 row

| number | name | current rarity | current types |
| --- | --- | --- | --- |
| 165 | Chansey | Rare Holo | V |

## Cosmic Eclipse — 1 row

| number | name | current rarity | current types |
| --- | --- | --- | --- |
| 143A | Togepi & Cleffa & Igglybuff | — | Tagteam |

## Journey Together — 2 rows

| number | name | current rarity | current types |
| --- | --- | --- | --- |
| 165 | Wailord | Illustration Rare | — |
| 172 | Iono’s Lillbolt | Special Illustration Rare | EX |

## MEP Black Star Promos — 9 rows

| number | name | current rarity | current types |
| --- | --- | --- | --- |
| 088 | Zarude | Illustration Rare | — |
| 101 | Nidorina | Illustration Rare | — |
| 102 | Victini | Illustration Rare | — |
| 103 | Zeraora | Illustration Rare | — |
| 106 | Ditto | Illustration Rare | — |
| 107 | Pikachu | Special Illustration Rare | EX |
| 108 | Espeon | Special Illustration Rare | EX |
| 109 | Pikachu | Special Illustration Rare | EX |
| 110 | Umbreon | Special Illustration Rare | EX |

## Silver Tempest — 1 row

| number | name | current rarity | current types |
| --- | --- | --- | --- |
| 178 | Regidrago | Rare Holo | V |

## Stellar Crown — 1 row

| number | name | current rarity | current types |
| --- | --- | --- | --- |
| 170 | Hydrapple | Special Illustration Rare | EX |

## White Flare — 2 rows

| number | name | current rarity | current types |
| --- | --- | --- | --- |
| 013 | Pansear | Reversed Holo | — |
| 013 | Pansear | Non-holo | — |

## Wizard Black Star Promos — 1 row

| number | name | current rarity | current types |
| --- | --- | --- | --- |
| — | Ancient Mew | Rare Holo | — |

## XY Black Star Promos — 1 row

| number | name | current rarity | current types |
| --- | --- | --- | --- |
| 223 | Venusaur | — | EX |
