...
const { colour, radius, brand } = await import(`data:text/javascript,${encodeURIComponent(js)}`);
...
/* Untitled UI's brand ramp, in this app's blue. Emitted after its own @theme
   body so these win, and outside `lines` so they do not also land in the :root
   block — nothing hand-written reads them, only Untitled UI's semantic layer
   does, and that lives in @theme. The ramp is the same in both themes on
   purpose; upstream does not redefine it in dark either. */
const brandLines = Object.entries(brand).map(
