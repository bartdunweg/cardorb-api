 ...
 *
 * What it deliberately does not generate: the rest of tokens.css. Spacing,
 * radii, blur, the shadows and the layout variables stay hand-written, because
 * nothing outside CSS consumes them and their arguments are prose that belongs
 * next to the value rather than in a TypeScript comment two files away.
 */
...
const { colour } = await import(`data:text/javascript,${encodeURIComponent(js)}`);
...
const css = `/* Generated by scripts/gen-tokens.mjs from lib/design/tokens.ts.
   ...
   pair updated on one side only is not expressible.
...
if (process.argv.includes("--check")) {
  ...
  console.log(`  ${OUT} is up to date (${lines.length} colours)`);
} else {
  writeFileSync(OUT, css);
  console.log(`  wrote ${OUT} — ${lines.length} colours`);
  try {
