...
export function parseExpansions(page: string): ScrydexExpansion[] {
  ...
  for (const m of page.matchAll(
    ...
    const code = m[3]!;
    if (!seen.has(code)) seen.set(code, { name: unescape(m[1]!), code, slug: m[2]! });
  }
  return [...seen.values()];
}
...
export function scrydexExpansionFor(
  expansions: ScrydexExpansion[],
  set: { id: string; name: string },
): ScrydexExpansion | null {
  const byCode = new Map(expansions.map((e) => [e.code, e]));
  const hand = BY_HAND[set.id];
  if (hand && byCode.has(hand)) return byCode.get(hand)!;
