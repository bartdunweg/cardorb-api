 ...
 *
 * English only, like the copy. A card the copy does not hold, or holds from before it kept sheets,
 * is null here, and the route asks TCGdex as it did (getCardDetail).
 */
...
export async function readCardSheet(tcgId: string): Promise<CatalogueCardSheet | null> {
  const db = adminClient();
  if (!db) return null;
  return catalogueCardSheet(db, tcgId).catch((err) => {
...
export function detailFromSheet({ card, set }: CatalogueCardSheet): CardDetail {
  return {
    ...
    set: set
      ? { id: set.id, name: set.name, logo: set.logo, total: set.total }
...
export async function eraRaritiesFromCopy(setId: string): Promise<string[] | null> {
  const db = adminClient();
  if (!db) return null;
  const rarities = await catalogueEraRarities(db, setId).catch((err) => {
