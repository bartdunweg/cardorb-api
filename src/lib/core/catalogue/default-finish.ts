...
/**
 * The same, for one card: out of the catalogue copy where it holds the card's printings
 * (card-sheet.ts, English and Japanese), asked of TCGdex only where it does not. The per-card
 * record is cached for a day. Anything that is not an answer (no id, an id TCGdex
