...
describe("the window", () => {
  it("lets maxPerWindow through and refuses the next", () => {
    ...
    expect([0, 1, 2].map((i) => limited("a", 1000 + i))).toEqual([false, false, false]);
  ...
  it("forgives once the window has passed", () => {
    ...
    expect(limited("a", 61_002)).toBe(false);
  });
  ...
  it("counts each key on its own", () => {
    ...
    expect(limited("b", 1002)).toBe(false);
  });
  ...
  it("keeps a hammered key bounded in memory", () => {
    ...
    expect(limited("a", 11_000)).toBe(true);
  });
});
...
describe("eviction, which is what a rotating flood meets", () => {
  it("drops keys whose hits have all aged out", () => {
    ...
    expect(limited("new", 10_000)).toBe(false);
    // The proof it swept rather than cleared: the new key still has its budget.
    for (let i = 0; i < 4; i++) expect(limited("new", 10_000 + i)).toBe(false);
    expect(limited("new", 10_005)).toBe(true);
  });
  ...
  it("does not un-throttle a blocked key when it has to make room", () => {
    ...
    expect(limited("blocked", 1004)).toBe(true);
    ...
    expect(limited("blocked", 1100)).toBe(true);
  });
  ...
  it("still caps memory when every key is blocked", () => {
    ...
    expect(limited("k-0", 1002)).toBe(true);
  });
});
