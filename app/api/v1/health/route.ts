 ...
 *
 * It answers 200 while the database is absent, because during the migration
 * that is the correct state rather than a fault — see COLLECTION_SOURCE in the
 * README. What it must never do is answer 200 while a configured database is
 * unreachable, which is what the `database` field is for once there is a client
 * to ask (phase 3).
 */
export const dynamic = "force-dynamic";

export async function GET() {
  const source = process.env.COLLECTION_SOURCE?.trim() || "notion";

  if (!configured()) {
    // Not a fault. During the migration a deployment without a database is the
    // expected state, and a monitor that goes red for it is a monitor nobody
    // reads by the second week.
    return answer({ ok: true, source, database: "absent" }, 200);
  }
  ...
  try {
    ...
    return answer({ ok: true, source, database: "reachable" }, 200);
  } catch (err) {
    ...
    return answer({ ok: false, source, database: "unreachable" }, 503);
  }
}
